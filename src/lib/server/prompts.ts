...
/**
 * Generates a simplified prompt for quick analysis (fallback mode).
 *
 * Used when full analysis fails or times out. This minimal prompt
 * requires only:
 * - Repository URL
 * - README content (if available)
 *
 * The structured output schema ensures consistent format even with
 * limited context. Returns basic fields: description, techStack, and
 * skillLevel. Useful for degraded functionality when full analysis
 * isn't possible.
 *
 * @param {string} repoUrl - Repository URL
 * @param {string} [readmeContent] - Optional README content
 * @returns {string} Simplified analysis prompt
 *
 * @example
 * ```typescript
 * // Use as fallback
 * try {
 *   const result = await analyzeRepository(context)
 * } catch (error) {
 ...
 * ```
 */
export function getQuickSummaryPrompt(repoUrl: string, readmeContent?: string): string {
  return `Provide a brief analysis of this repository:
...
Based on this limited information, provide:
- A brief description of what the project likely does
- Technologies you can identify from the README
- An estimated skill level required`
}
